...
async function initAnalysis() {
    ...
    for (stox in NIFTY50) {
        if (!OC_DIRECTION['TCS']) {
            OC_DIRECTION['TCS'] = await analyseOptionChain(stox)
        }
    }
    for (stox in BNF) {
        if (!OC_DIRECTION['TCS']) {
    ...
    let niftyIDWeightage = 0
    let niftyPosWeightage = 0
    let niftyTableStr = ""
    for (stox in NIFTY50) {
        let stoxWeight = getNumber(NIFTY50[stox])
        niftyStat.idWeight += OC_DIRECTION_OUTPUT[stox].intraDay * stoxWeight
        niftyStat.posWeight += OC_DIRECTION_OUTPUT[stox].positional * stoxWeight
        OC_DIRECTION_OUTPUT[stox].intraDay == 1 ? niftyStat.idUpCount++ : niftyStat.idDownCount++
        OC_DIRECTION_OUTPUT[stox].positional == 1 ? niftyStat.posUpCount++ : niftyStat.posDownCount++
        niftyIDWeightage += OC_DIRECTION_OUTPUT[stox].intraDay * stoxWeight
        niftyPosWeightage += OC_DIRECTION_OUTPUT[stox].positional * stoxWeight
        niftyTableStr += `<tr>
            <td>${stox}</td><td>${OC_DIRECTION_OUTPUT[stox].intraDay == 1 ? '🟢' : '🔻'}(${stoxWeight})</td>
            <td>${stox}</td><td>${OC_DIRECTION_OUTPUT[stox].positional == 1 ? '🟢' : '🔻'}(${stoxWeight})</td>
        </tr>`
    }

    console.log(parseFloat(niftyIDWeightage).toFixed(2), parseFloat(niftyPosWeightage).toFixed(2))
    console.log(parseFloat(niftyStat.idWeight).toFixed(2), parseFloat(niftyStat.posWeight).toFixed(2))
    console.log(niftyStat.idUpCount, niftyStat.idDownCount, niftyStat.posUpCount, niftyStat.posDownCount)
    let bnfStat = {
        idWeight: 0,
        posWeight: 0,
        idUpCount: 0,
    ...
    let bnfIDWeightage = 0;
    let bnfPosWeightage = 0;
    let bnfTableStr = ""
    for (stox in BNF) {
        let stoxWeight = getNumber(BNF[stox])
        bnfStat.idWeight += OC_DIRECTION_OUTPUT[stox].intraDay * stoxWeight
        bnfStat.posWeight += OC_DIRECTION_OUTPUT[stox].positional * stoxWeight
        OC_DIRECTION_OUTPUT[stox].intraDay == 1 ? bnfStat.idUpCount++ : bnfStat.idDownCount++
        OC_DIRECTION_OUTPUT[stox].positional == 1 ? bnfStat.posUpCount++ : bnfStat.posDownCount++
        bnfIDWeightage += OC_DIRECTION_OUTPUT[stox].intraDay * stoxWeight
        bnfPosWeightage += OC_DIRECTION_OUTPUT[stox].positional * stoxWeight
        console.log(stox, OC_DIRECTION_OUTPUT[stox].intraDay, stoxWeight)
        bnfTableStr += `<tr>
            <td>${stox}</td><td>${OC_DIRECTION_OUTPUT[stox].intraDay == 1 ? '🟢' : '🔻'}(${stoxWeight})</td>
            <td>${stox}</td><td>${OC_DIRECTION_OUTPUT[stox].positional == 1 ? '🟢' : '🔻'}(${stoxWeight})</td>
        </tr>`
    }
    console.log(parseFloat(bnfIDWeightage).toFixed(2), parseFloat(bnfPosWeightage).toFixed(2))
    console.log(parseFloat(bnfStat.idWeight).toFixed(2), parseFloat(bnfStat.posWeight).toFixed(2))
    console.log(bnfStat.idUpCount, bnfStat.idDownCount, bnfStat.posUpCount, bnfStat.posDownCount)
    console.log(bnfTableStr)
}
...
async function analyseOptionChain(stox) {
    ...
    // console.log('ATM: ', optionChain[atmRowIndex][COL_MAP.STRIKE_PRICE])
    for (let rowIndex = atmRowIndex - 1; rowIndex <= atmRowIndex + STRIKES_FROM_ATM; rowIndex++) {
        if (optionChain[rowIndex]) {
            // console.log('CALL: ', optionChain[rowIndex][COL_MAP.STRIKE_PRICE])
    ...
    for (let rowIndex = atmRowIndex - STRIKES_FROM_ATM; rowIndex <= atmRowIndex + 1; rowIndex++) {
        if (optionChain[rowIndex]) {
            // console.log('PUT: ', optionChain[rowIndex][COL_MAP.STRIKE_PRICE])
    ...
    let intraDayPCR = '∞'
    if (callOIChgSum > 0) {
        intraDayPCR = parseFloat(putOIChgSum / calcallOIChgSumlOISum).toFixed(2)
    }
    return {
