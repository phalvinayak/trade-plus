...
function updateChipsHawa() {
  ...
  if (!futPrice) {
    return;
  } // No future price can't do anything
  ...
  if (strikeDiff === 0) {
    ...
    let atmStrike = $("#current-atm-row")
      .find(`div.rt-td:nth-child(${strikeCol})`)
      .find("span")
      .text()
      .trim();
  ...
  $("#oc-table-body div.rt-tr-group").each((i, row) => {
    ...
    let strikePrice = $strikeCell.find("[class*=StrikeWrapper]").text().trim();
    ...
    let ceOI = $callOICell
      .find(`div[class*='style__OiprogressBarValue']`)
      .text()
      .trim();
    let peOI = $putOICell
      .find(`div[class*='style__OiprogressBarValue']`)
    ...
    if (
      highlightStrike[0] == strikePrice ||
      highlightStrike[1] == strikePrice
    ) {
...
function highlightCell(rowIndex, colIndex, className = null) {
  ...
  $row = $(
    `#oc-table-body div.rt-tr-group:nth-child(${rowIndex}) .rt-tr div.rt-td:nth-child(${colIndex})`
  ).addClass(className);
}
...
function updateTableHeader(callCol, putCol, strikeCol) {
  if ($("#oc-table-head .chips-hawa-price").length == 0) {
    ...
    $(`#oc-table-head .rt-tr>div:nth-child(${callCol})`).prepend(
      `<div class='chips-hawa-price chips'>CHIPS</div>`
    );
    $(`#oc-table-head .rt-tr>div:nth-child(${callCol})`).append(
      `<div class='chips-hawa-price hawa'>HAWA</div>`
    );

    // PUT side Chips Hawa
    $(`#oc-table-head .rt-tr>div:nth-child(${putCol})`).prepend(
      `<div class='chips-hawa-price chips'>CHIPS</div>`
    );
    $(`#oc-table-head .rt-tr>div:nth-child(${putCol})`).append(
      `<div class='chips-hawa-price hawa'>HAWA</div>`
    );

    // Strike Price PCR
    $(`#oc-table-head .rt-tr>div:nth-child(${strikeCol})`).append(
      `<div class='chips-hawa-pcr-ratio'>PCR</div>`
    );
    $(`#oc-table-head .rt-tr>div:nth-child(${strikeCol})`).prepend(
      `<div class='chips-hawa-pcr-ratio p-hawa'>%Hawa</div>`
    );
  }
}
...
function udpateStrikePCR(peStrike, ceStrike, $cell) {
  ...
  if ($cell.find(".chips-hawa-pcr-ratio.p-hawa").length) {
    $cell
      .find(".chips-hawa-pcr-ratio.p-hawa")
      .text(`${parseFloat(pHawa).toFixed(2)}%`);
  } else {
    $cell.prepend(
      `<div class='chips-hawa-pcr-ratio p-hawa'>${parseFloat(pHawa).toFixed(
        2
      )}%</div>`
    );
  }
  ...
  let pcr = "-n/a-";
  if (
    optionChainPrice[peStrike]["oi"] != 0 &&
    optionChainPrice[ceStrike]["oi"] != 0
  ) {
    pcr = parseFloat(
      optionChainPrice[peStrike]["oi"] / optionChainPrice[ceStrike]["oi"]
    ).toFixed(2);
  }
...
function renderOIChange(ceOISum, peOISum, ceOISumRange, peOISumRange) {
  ...
  $OICard.find(".diff_item").text(`${parseFloat(peOISumRange).toFixed(
    2
  )} - ${parseFloat(ceOISumRange).toFixed(2)}
	= ${parseFloat(OIDiff).toFixed(2)}L (PCR: ${parseFloat(
    peOISumRange / ceOISumRange
  ).toFixed(2)})`);
  if (OIDiff < 0) {
  ...
  $OICard.find(".all-oi").text(`Over all OI diff: ${parseFloat(peOISum).toFixed(
    2
  )} - ${parseFloat(ceOISum).toFixed(2)} 
	= ${parseFloat(peOISum - ceOISum).toFixed(2)}L (PCR: ${parseFloat(
    peOISum / ceOISum
  ).toFixed(2)})`);
}
...
function getFuturePrice() {
  if ($("[class^=style__TickerButtonText]").length) {
    let futPrice = $("[class^=style__TickerButtonText]")
      .contents()
      .filter(function () {
...
function getStrikeDiff(strikeCol) {
  let $strikeCell1 = $(
    `#oc-table-body div.rt-tr-group:nth-child(1) .rt-tr div.rt-td:nth-child(${strikeCol})`
  );
  let strikePrice1 = $strikeCell1.find("span").text().trim();

  let $strikeCell2 = $(
    `#oc-table-body div.rt-tr-group:nth-child(2) .rt-tr div.rt-td:nth-child(${strikeCol})`
  );
...
function initMenuBar() {
  ...
  $(".chips-hawa-cards.cards.oi button, .chips-hawa-menu a.oi").on(
    "click",
    (e) => {
      e.preventDefault();
      e.stopPropagation();
  ...
  $(".chips-hawa-cards.cards.settings button, .chips-hawa-menu a.settings").on(
    "click",
    (e) => {
      e.preventDefault();
      e.stopPropagation();
...
function ltpCalculator(
  ...
  if (type == "error") {
    ...
    if (parseFloat(putPrice) < parseFloat(callPrice)) {
      ...
      output =
        spotPrice +
        (parseFloat(putPrice) - parseFloat(callPrice)) /
          (callDelta + putDelta) +
        0.05;
      output = isNumeric(parseFloat(output).toFixed(2))
    ...
    if (parseFloat(callPrice) < parseFloat(putPrice)) {
      ...
      output =
        spotPrice -
        (parseFloat(callPrice) - parseFloat(putPrice)) /
          (callDelta + putDelta) -
        0.05;
      output = isNumeric(parseFloat(output).toFixed(2))
        ? parseFloat(output).toFixed(2)
...
function ltpCalculatorUI(e) {
  ...
  let strikePrice = parseFloat(
    $strikeCell.find("[class*=StrikeWrapper]").text().trim()
  ).toFixed(2);
  ...
  if (optionChainPrice[putStrikeKey] && optionChainPrice[callStrikeKey]) {
    // Pairing put strike present
    callOptionData = optionChainPrice[callStrikeKey];
    putOptionData = optionChainPrice[putStrikeKey];
    ltpCalculator(
      type,
      callOptionData.price,
      callOptionData.delta,
      putOptionData.price,
      putOptionData.delta,
      selectedOptionData.strike,
      sl
    );
  } else {
...
function resetView(view) {
  ...
  if (view == "chips") {
    ...
    const $header = $("#oc-table-head");
    $header.find("div.chips-hawa-price").remove();
    $header.find("div.chips-hawa-pcr-ratio").remove();
    $container.find("div.chips-hawa-price").remove();
    $container.find("div.chips-hawa-pcr-ratio").remove();
    $container
      .find(".rt-td.chips-hawa-highlight")
      .removeClass("chips-hawa-highlight");
    optionChainPrice = {};
  } else if (view == "ltp") {
    $("body").removeClass("chips-hawa-ltp");
    $("body").addClass("chips-hawa-nk");
    $container
      .find(".chips-hawa-highlight-cell")
      .removeClass("chips-hawa-highlight-cell");
    $container
      .find(".chips-hawa-imaginary-line")
      .removeClass("chips-hawa-imaginary-line");
    $container
      .find(".chips-hawa-highlight-int-cell")
      .removeClass("chips-hawa-highlight-int-cell");
...
function updateLTP() {
  ...
  $("#oc-table-body div.rt-tr-group").each((i, row) => {
    let $row = $(row);
    let $strikeCell = $row.find(
      `.rt-tr div.rt-td:nth-child(${colPos.strikeCol})`
    );
    ...
    let $callOICell = $row.find(
      `.rt-tr div.rt-td:nth-child(${colPos.callOICol})`
    );
    let $putOICell = $row.find(
      `.rt-tr div.rt-td:nth-child(${colPos.putOICol})`
    );
    let $putVolumeCell = $row.find(
      `.rt-tr div.rt-td:nth-child(${colPos.putVolume})`
    );
    let $callVolumeCell = $row.find(
      `.rt-tr div.rt-td:nth-child(${colPos.callVolume})`
    );
    let $callDeltaCell = $row.find(
      `.rt-tr div.rt-td:nth-child(${colPos.callDelta})`
    );
    let $putDeltaCell = $row.find(
      `.rt-tr div.rt-td:nth-child(${colPos.putDelta})`
    );
    let $callOIChangeCell = $row.find(
      `.rt-tr div.rt-td:nth-child(${colPos.callOIChange})`
    );
    let $putOIChangeCell = $row.find(
      `.rt-tr div.rt-td:nth-child(${colPos.putOIChange})`
    );
    ...
    // Get Call Price of the same option
    let cePrice = $callCell.find("[class*=LtpValue]").text().split(" ")[0];
    cePrice = parseFloat(cePrice).toFixed(2);
    optionChainPrice[callStrike]["price"] = cePrice;

    // Get Put Price of the same option
    let pePrice = $putCell.find("[class*=LtpValue]").text().split(" ")[0];
    pePrice = parseFloat(pePrice).toFixed(2);
    optionChainPrice[putStrike]["price"] = pePrice;

    // Get OI values from the cell
    let callOI = $callOICell
      .find(`div[class*='style__OiprogressBarValue']`)
      .text()
      .trim();
    optionChainPrice[callStrike]["oi"] = isNumeric(callOI)
      ? parseFloat(callOI)
      : 0;
    let putOI = $putOICell
      .find(`div[class*='style__OiprogressBarValue']`)
      .text()
      .trim();
    optionChainPrice[putStrike]["oi"] = isNumeric(putOI)
      ? parseFloat(putOI)
      : 0;

    // Get the volume from the cell
    let callVolume = $callVolumeCell.text().trim();
    optionChainPrice[callStrike]["volume"] = isNumeric(callVolume)
      ? parseFloat(callVolume)
      : 0;
    let putVolume = $putVolumeCell.text().trim();
    optionChainPrice[putStrike]["volume"] = isNumeric(putVolume)
      ? parseFloat(putVolume)
      : 0;

    // Get the delta from the cell
    let callDelta = $callDeltaCell.text().trim();
    optionChainPrice[callStrike]["delta"] = isNumeric(callDelta)
      ? parseFloat(callDelta)
      : 0;
    let putDelta = $putDeltaCell.text().trim();
    optionChainPrice[putStrike]["delta"] = isNumeric(putDelta)
      ? parseFloat(putDelta) * -1
      : 0;

    // Get the OIChange from the cell
    let callOIChangePer = $callOIChangeCell.text().trim().replace("%", "");
    optionChainPrice[callStrike]["oiChange"] = isNumeric(callOIChangePer)
      ? parseFloat(callOIChangePer)
      : 0;
    let putOIChangePer = $putOIChangeCell.text().trim().replace("%", "");
    optionChainPrice[putStrike]["oiChange"] = isNumeric(putOIChangePer)
      ? parseFloat(putOIChangePer)
      : 0;
    ...
    if (callOIChange > 0 && putOIChange > 0) {
      if (callOIChange > putOIChange) {
        OIChangeRatio = `${parseFloat(callOIChange / putOIChange).toFixed(
          2
        )}:1`;
      } else {
        OIChangeRatio = `1:${parseFloat(putOIChange / callOIChange).toFixed(
          2
        )}`;
      }
    } else if (callOIChange < 0 && putOIChange < 0) {
      if (callOIChange > putOIChange) {
        OIChangeRatio = OIChangeRatio = `1:${parseFloat(
          (putOIChange * -1) / (callOIChange * -1)
        ).toFixed(2)}`;
      } else {
        OIChangeRatio = `${parseFloat(
          (callOIChange * -1) / (putOIChange * -1)
        ).toFixed(2)}:1`;
      }
    } else {
      OIChangeRatio = `∞`;
    ...
    if ($strikeCell.find(".chips-hawa-oi-ratio").length) {
      ...
      $strikeCell.append(
        `<div class='chips-hawa-oi-ratio'>${OIChangeRatio}</div>`
      );
    }
    ...
    if (
      SHOW_LTP &&
      $callCell.find("button.chips-hawa-ltp-trigger").length == 0
    ) {
      let $btn = $("<button>", {
    ...
    if (
      SHOW_LTP &&
      $putCell.find("button.chips-hawa-ltp-trigger").length == 0
    ) {
      let $btn = $("<button>", {
        class: "chips-hawa-ltp-trigger",
        text: "LTP",
  ...
  $(".chips-hawa-highlight-int-cell").removeClass(
    "chips-hawa-highlight-int-cell"
  );
  ...
  $(`#oc-table-body div.rt-tr-group:nth-child(${iLineindex2})`).addClass(
    "chips-hawa-imaginary-line"
  );
  ...
  do {
    if (
      optionChainPrice[maxCallVolumeStrike].volume <
      optionChainPrice[callStrikeKey].volume
    ) {
      maxCallVolumeStrike = callStrikeKey;
    }
    if (
      optionChainPrice[maxCallOIStrike].oi < optionChainPrice[callStrikeKey].oi
    ) {
      maxCallOIStrike = callStrikeKey;
    }
    if (
      optionChainPrice[maxCallOIChangeStrike].oiChange <
      optionChainPrice[callStrikeKey].oiChange
    ) {
      maxCallOIChangeStrike = callStrikeKey;
    }
    callStrikePrice =
      strikePriceOrder[optionChainPrice[callStrikeKey].index + 1];
  ...
  highlightCell(
    optionChainPrice[maxCallOIChangeStrike].index,
    colPos.callOIChange,
    "chips-hawa-highlight-int-cell"
  );
  ...
  do {
    if (
      optionChainPrice[maxPutVolumeStrike].volume <
      optionChainPrice[putStrikeKey].volume
    ) {
      maxPutVolumeStrike = putStrikeKey;
    }
    if (
      optionChainPrice[maxPutOIStrike].oi < optionChainPrice[putStrikeKey].oi
    ) {
      maxPutOIStrike = putStrikeKey;
    }
    if (
      optionChainPrice[maxPutOIChangeStrike].oiChange <
      optionChainPrice[putStrikeKey].oiChange
    ) {
  ...
  highlightCell(
    optionChainPrice[maxPutOIChangeStrike].index,
    colPos.putOIChange,
    "chips-hawa-highlight-int-cell"
  );
}
...
function getElementPosition($element) {
  let leftPos =
    $element[0].getBoundingClientRect().left + $(window)["scrollLeft"]();
  let rightPos =
    $element[0].getBoundingClientRect().right + $(window)["scrollLeft"]();
  let topPos =
    $element[0].getBoundingClientRect().top + $(window)["scrollTop"]();
  let bottomPos =
    $element[0].getBoundingClientRect().bottom + $(window)["scrollTop"]();
