...
let stocks = [];
const LINK_SELECTOR = "a[href*='/stocks-new?']";

function getInterval() {
  let availableTf = [];
  if ($("span.atlas-offset").length == 0) {
    return "D";
  }
  let leastTF = "M";
  $("span.atlas-offset").each((i, e) => {
    if (
      $(e).closest(".atlas-filter-grabbable").find(".fa-toggle-off").length == 0
    ) {
...
function linkify() {
  ...
  $(
    `#root ${LINK_SELECTOR}, .panel-body ${LINK_SELECTOR}, #atlas-grid-layout ${LINK_SELECTOR}`
  ).each((i, e) => {
    let $e = $(e);
    if (
      $e.parent().find(LINK_SELECTOR).length ==
      $e.parent().find(".chips-hawa-tv_link").length
    ) {
      return;
    }
    let nseStock = getStock(e.href);
...
function copyScrips(backtest = false) {
  stocks = [];
  if (backtest) {
    $(`#backtest-table-data ${LINK_SELECTOR}`).each((i, e) => {
      let nseStock = getStock(e.href);
      if (!stocks.includes(nseStock)) {
        stocks.push(nseStock);
      }
    ...
    $(`.scanner ${LINK_SELECTOR}`).each((i, e) => {
      let nseStock = getStock(e.href);
      if (!stocks.includes(nseStock)) {
        stocks.push(nseStock);
      }
...
function getStock(link) {
  ...
  let stock = urlParams.get("symbol");
  console.log(link, stock);
  stock = stock.replace(/\&/g, "_");
  console.log(stock);
  return `NSE:${stock}`;
}

let linkifyBtnMarkup =
  '<button class="btn btn-default btn-primary chips-hawa-linkify" type="button"><span>Linkify</span></button>';
let saveBtnMarkup =
  '<button class="btn btn-default btn-primary chips-hawa-copy" type="button"><span>Copy List</span></button>';
...
$("#backtest-container .inked select").after(
  `<button class="btn btn-default btn-primary chips-hawa-linkify chips-hawa-copy chips-hawa-backtest" style="margin-left:0 !important" type="button"><span>Copy List</span></button>`
);
$("#backtest-container .inked select").after(
  `<button class="btn btn-default btn-primary chips-hawa-linkify chips-hawa-backtest" type="button"><span>Linkify</span></button>`
);
...
function registerBindEvent() {
  $("#DataTables_Table_0, #backtest-table-data").bind(
    "DOMSubtreeModified",
    debounce(linkify, 250)
  );
}
